...
with open("CosmicFusionExport.tsv", 'r') as hin:
    for line in hin:
        F = line.rstrip('\n').split('\t')
        if F[11] == "": continue
...
with xlrd.open_workbook("onc2014406x2.xls") as book:
...
with xlrd.open_workbook("nbt.3080-S7.xls") as book:
...
with xlrd.open_workbook("nbt.3080-S8.xls") as book:
    sheet1 = book.sheet_by_index(0)
    for i in range(2, sheet1.nrows):
...
with xlrd.open_workbook("ncomms5846-s3.xlsx") as book:
    sheet1 = book.sheet_by_index(0)
    for i in range(2, sheet1.nrows):
