...
import pysam
from . import my_seq
from nccbreak.long_read_validate import ssw_check
...
def long_read_validate_by_alignment(sv_file, output_file, bam_file, reference, debug, validate_sequence_length = 200, score_ratio_thres = 1.4, start_pos_thres = 0.2, end_pos_thres = 0.8, var_ref_margin_thres = 10):
    ...
    with open(sv_file, 'r') as hin:
        for line in hin:
            ...
            for read in bam_ps.fetch(tchr1, tpos1 - 100, tpos1 + 100):

                if read.qname not in rname2key: rname2key[read.qname] = []
                rname2key[read.qname].append(key)

            for read in bam_ps.fetch(tchr2, tpos2 - 100, tpos2 + 100):
    ...
    with open(sv_file, 'r') as hin:
        ...
        for line in hin:
            ...
            # reference_local_seq
            reference_local_seq_1 = reference_fasta.fetch(tchr1, tpos1 - validate_sequence_length - 1, tpos1 + validate_sequence_length - 1) 
            reference_local_seq_2 = reference_fasta.fetch(tchr2, tpos2 - validate_sequence_length - 1, tpos2 + validate_sequence_length - 1)  

            # variant_seq
            variant_seq = ""
            if tdir1 == '+':
                tseq = reference_fasta.fetch(tchr1, tpos1 - validate_sequence_length - 1, tpos1 - 1)
            else:  
            ...
            if tdir2 == '-':
                ...
                tseq = reference_fasta.fetch(tchr2, tpos2 - validate_sequence_length - 1, tpos2 - 1)
                tseq = my_seq.reverse_complement(tseq)
